...
# --- 1. CONFIGURACIÓN ---
st.set_page_config(page_title="Cartera Agirre & Uranga v54", layout="wide")
...
# --- 3. DATOS MAESTROS (CARGA INICIAL COMPLETA) ---
...
ARCHIVO_V54 = "cartera_ Aguirre_Uranga_v54_final.csv"

if 'df' not in st.session_state:
    try:
        st.session_state.df = pd.read_csv(ARCHIVO_V54)
    except:
        st.session_state.df = pd.DataFrame(cargar_datos_iniciales())
        st.session_state.df.to_csv(ARCHIVO_V54, index=False)
...
# Selector para recuperar la "información desaparecida"
...
def mostrar_seccion(tipo):
    ...
    if vista == "Resumen por Activo":
        res = subset.groupby(['Nombre', 'Broker', 'Moneda']).agg({'Cant': 'sum', 'Coste': 'sum', 'Valor Mercado': 'sum', 'Ganancia (€)': 'sum', 'P_Act': 'first'}).reset_index()
    else:
        res = subset.copy()
    ...
    # Columnas a mostrar
    cols = ['Broker', 'Nombre', 'Cant', 'Coste', 'Valor Mercado', 'P_Act', 'Ganancia', 'Rent. %']
    if vista == "Detalle de Todas las Compras": cols.insert(0, 'Fecha')
    
    st.dataframe(res.rename(columns={'Cant': 'Cant.', 'Coste': 'Invertido', 'Valor Mercado': 'Val. Act.', 'P_Act': 'Precio Unit.'})[cols], use_container_width=True, hide_index=True)
...
# --- 6. GESTIÓN (RECUPERADA) ---
st.divider()
with st.expander("🛠️ Gestión: Añadir o Eliminar Operaciones"):
    st.write("Añade una nueva compra aquí abajo:")
    with st.form("nuevo"):
        ...
        f_nombre = col5.text_input("Nombre")
        f_cant = col6.number_input("Cant.", format="%.4f")
        f_coste = col7.number_input("Inversión €", format="%.2f")
        f_moneda = col8.selectbox("Moneda", ["EUR", "USD"])
...
with st.sidebar:
    ...
    if st.button("♻️ RESET TOTAL (Fábrica)"):
        st.session_state.df = pd.DataFrame(cargar_datos_iniciales())
        st.session_state.df.to_csv(ARCHIVO_V54, index=False)
        st.rerun()
