...
st.set_page_config(page_title="Gestor de Inversiones Desglosado", layout="wide")

# --- 1. CARGA DE TODAS LAS OPERACIONES (Extraído de tus informes) ---
def cargar_historial_completo():
    return [
        ...
        # FONDOS RENTA 4 (Desglose individual)
...
# Nuevo nombre de archivo para forzar actualización de datos
ARCHIVO_CSV = "cartera_desglosada_final.csv"

if 'df_cartera' not in st.session_state:
    try:
        st.session_state.df_cartera = pd.read_csv(ARCHIVO_CSV)
    except:
        st.session_state.df_cartera = pd.DataFrame(cargar_historial_completo())
...
# --- 4. INTERFAZ ---
st.title("🏦 Cuadro de Mando Patrimonial: Historial Detallado")
...
c3.metric("G/P Total Global", f"{(b_acc + b_fon):,.2f} €")

st.divider()

def mostrar_seccion_desglosada(titulo, filtro_tipo):
    st.header(f"💼 {titulo}")
    df_sub = df[df['Tipo'] == filtro_tipo]
    
    # Tabla 1: RESUMEN (Agrupado por Nombre)
    ...
    st.dataframe(resumen.style.format({
        "Cant": "{:.2f}", "Coste": "{:.2f} €", "Valor_Actual": "{:.2f} €", "G/P": "{:.2f} €"
    }), use_container_width=True)
    
    # Tabla 2: HISTORIAL (Desplegable con fechas)
    st.subheader(f"📜 Historial de Compras ({titulo})")
    for nombre in df_sub['Nombre'].unique():
        compras = df_sub[df_sub['Nombre'] == nombre].sort_values(by='Fecha', ascending=False)
        with st.expander(f"Ver cada compra de: {nombre}"):
            st.table(compras[['Fecha', 'Cant', 'Coste', 'P_Act', 'G/P']].style.format({
                "Cant": "{:.4f}", "Coste": "{:.2f} €", "P_Act": "{:.4f} €", "G/P": "{:.2f} €"
            }))

# RENDERIZADO
mostrar_seccion_desglosada("Acciones", "Acción")
st.divider()
mostrar_seccion_desglosada("Fondos de Inversión", "Fondo")
...
if st.sidebar.button("🚨 Resetear a Datos de Fábrica"):
    st.session_state.df_cartera = pd.DataFrame(cargar_historial_completo())
    st.session_state.df_cartera.to_csv(ARCHIVO_CSV, index=False)
    st.rerun()
