...
st.markdown("""
    ...
    .card-label { font-size: 0.85rem; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; color: #6b7280; }
...
if check_password():
    ...
    # --- 4. FUNCIONES DE APOYO (PUNTO 2 APLICADO) ---
    ...
    # --- 5. BASES DE DATOS (RESTAURADO COMPLETO) ---
    ...
    # --- 7. BARRA LATERAL (PUNTOS 1 Y 3 + FIX ERROR) ---
    with st.sidebar:
        st.markdown("### 🏦 Administración")
        if st.button("🔄 Sincronizar Bolsa", use_container_width=True):
            try:
                # Sincronización FX (Punto 3)
                rate_data = yf.download("EURUSD=X", period="1d", progress=False)
                rate = float(rate_data["Close"].iloc[-1]) if not rate_data.empty else 1.09
                st.session_state.rate_aguirre = rate
                
                # Sincronización masiva (Punto 1)
                tkrs = st.session_state.df_cartera[st.session_state.df_cartera['Tipo'] == "Acción"]['Ticker'].unique().tolist()
                if tkrs:
                    data_bolsa = yf.download(tkrs, period="1d", progress=False)['Close']
                    ahora = datetime.now().strftime("%d/%m/%Y %H:%M")
                    
                    for i, row in st.session_state.df_cartera.iterrows():
                        if row['Tipo'] == "Acción":
                            ticker = row['Ticker']
                            # Extracción segura de precio (Float)
                            p_raw = data_bolsa[ticker].iloc[-1] if len(tkrs) > 1 else data_bolsa.iloc[-1]
                            if pd.notnull(p_raw):
                                p_num = float(p_raw)
    ...
    inv_total, val_total = df_v['Coste'].sum(), df_v['Valor Mercado'].sum()
    ben_total = val_total - inv_total
    rent_total = (ben_total / inv_total * 100 if inv_total > 0 else 0)
    
    st.markdown(f"""
        <div class="metric-container">
            <div class="custom-card"><div class="card-label">Capital Invertido</div><div class="card-value">{inv_total:,.2f} €</div></div>
            <div class="custom-card"><div class="card-label">Valor de Mercado</div><div class="card-value">{val_total:,.2f} €</div></div>
            <div class="custom-card highlight-card"><div class="card-label">Beneficio Latente Total</div>
                <div class="card-value">{ben_total:,.2f} € <span class="pct-badge {"pct-pos" if rent_total >= 0 else "pct-neg"}">{rent_total:+.2f}%</span></div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    st.divider()

    # --- 10. TABLAS DE POSICIONES (PUNTO 2 APLICADO) ---
    def mostrar_seccion(tit, tipo_filtro, icon):
        ...
        res_disp = res.rename(columns={'Cant': 'Cant/Part.', 'Coste': 'Inversión', 'Valor Mercado': 'Valor (€)', 'Ult_Val': 'Última Val.'})

        with st.container(border=True):
            st.dataframe(
                res_disp[['Broker', 'Nombre', 'Cant/Part.', 'Inversión', 'Valor (€)', 'Precio Actual', 'Beneficio (€/$)', 'Rentabilidad (%)', 'Última Val.']]
    ...
    st.markdown(f"""<div style='text-align: center; background: #111827; padding: 25px; border-radius: 12px; color: white; margin: 20px 0;'>
        <span style='font-size: 14px; text-transform: uppercase; color: #9ca3af;'>Suma Total Aportado</span><br>
        <span style='font-size: 32px; font-weight: 800;'>{total_a + total_x:,.2f} €</span></div>""", unsafe_allow_html=True)
    st.divider()
    ...
    with tabs[1]:
        g1, g2 = st.columns(2)
        g1.plotly_chart(px.pie(df_v[df_v['Tipo']=='Acción'], values='Valor Mercado', names='Nombre', title="Acciones", hole=0.4, color_discrete_sequence=colors), use_container_width=True)
        g2.plotly_chart(px.pie(df_v[df_v['Tipo']=='Fondo'], values='Valor Mercado', names='Nombre', title="Fondos", hole=0.4, color_discrete_sequence=colors), use_container_width=True)
