...
ARCHIVO_CSV = "cartera_ Aguirre_Uranga_v49.csv"
...
c3.metric("Ganancia TOTAL", f"{g_tot:,.2f} €", delta=f"{g_tot*rt:,.2f} $", delta_color="off")
...
def mostrar_seccion(titulo, filtro):
    ...
    res = df_sub.groupby(['Nombre', 'Broker', 'Moneda']).agg({'Cant':'sum','Coste':'sum','Valor Mercado':'sum','Ganancia (€)':'sum', 'P_Act': 'first'}).reset_index()
    res['Rentabilidad %'] = (res['Ganancia (€)'] / res['Coste'] * 100)
    ...
    if filtro == "Fondo":
        ...
        edited = st.data_editor(
            res_ed[['Broker', 'Nombre', 'Cantidad / Part.', 'Dinero Invertido', 'Valor Mercado', 'Precio Actual', 'Ganancia Total', 'Rentabilidad %']],
            use_container_width=True, hide_index=True,
            disabled=['Broker', 'Nombre', 'Cantidad / Part.', 'Dinero Invertido', 'Valor Mercado', 'Ganancia Total', 'Rentabilidad %']
        )
        for i, row in edited.iterrows():
            st.session_state.df_cartera.loc[st.session_state.df_cartera['Nombre'] == row['Nombre'], 'P_Act'] = row['Precio Actual']
    else:
        st.dataframe(res.rename(columns={'Cant': 'Cant.', 'Coste': 'Invertido'})[['Broker', 'Nombre', 'Cant.', 'Invertido', 'Valor Mercado', 'Precio', 'Ganancia Total', 'Rentabilidad %']], use_container_width=True, hide_index=True)
